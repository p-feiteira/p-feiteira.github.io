...
export default function InfoCard({Icon ,title, children, className, compact = false, alignLeft = false }: CardProps) {
    return <Card className={`w-full bg-muted-foreground/5 dark:bg-muted/10 transition-all duration-300 my-3 hover:shadow-lg hover:-translate-y-1 ${compact ? 'h-20 sm:h-24' : ''} ${className ?? ''} ${compact ? 'h-full' : ''}`}>
